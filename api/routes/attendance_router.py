...
def toggle_attendance(
    ...
    today = now.date()
    attendance = get_db_attendance_by_date(current_user.id, today, db)
    if not attendance:
        ...
        shift_start_dt = datetime.combine(today, shift.start_time).replace(
            tzinfo=timezone.utc
        )
        grace_delta = timedelta(minutes=shift.grace_period_minutes)
        cut_off_time = shift_start_dt + grace_delta
        is_late = now > cut_off_time
    ...
    if open_session:
        ...
        duration = now - open_session.clock_in
