...
const clearButtonStyles = {
  position: "absolute",
  right: 32, // Flushed right beside the native select chevron icon
  top: "50%",
...
export default function ManageHolidaysPage() {
  ...
  // Unified form engine initial state assignments
  ...
  // Bulk synchronizer logic (GET: /admin/holiday/ & /admin/location/)
          ...
          >
            ...
            >
              <Stack spacing={3} sx={{ p: 0.5 }}>
                ...
                {/* 2. Calendar Date Field selection row */}
                <Controller
                ...
                {/* 3. Refactored Premium Multi-Select Dropdown field context block */}
                <Box>
                  <Typography
                    variant="caption"
                    sx={{
                  ...
                  <Controller
                    ...
                    render={({ field, fieldState: { error } }) => {
                      ...
                      // High Performance intercept change handler tracking placeholder triggers
                      const handleCustomChange = (event: any) => {
                        const values = event.target.value as string[];

                        // Check if the user selected/interacted with the master toggle placeholder item
                        if (values.includes("ALL")) {
                          if (isAllSelected) {
                            field.onChange([]); // Flush completely
                          } else {
                            field.onChange(locations.map((loc) => loc.id)); // Bulk load all IDs
                          }
                        } else {
                          field.onChange(values); // Standard routing logic pass-through
                        }
                      };
                      ...
                      return (
                        <Box sx={{ position: "relative" }}>
                          ...
                          >
                            {/* 3a. Certified Master Select All row toggle configuration */}
                            {locations.length > 0 && (
                              <MenuItem
                                value="ALL"
                                sx={{
                            ...
                            {/* 3b. Individual options arrays data layout mapping lists */}
                            {locations.map((loc) => (
                              <MenuItem
                                key={loc.id}
                                value={loc.id}
                          ...
                          {/* Quick erase selection close anchor button trigger links */}
                          {selectedIds.length > 0 && (
                            <IconButton
                              size="small"
                ...
                {/* 4. Active Status toggle switch component card */}
                <Card
                  variant="outlined"
                  sx={{
                    p: 2,
